...
const ManageAttendance = () => {
  ...
  const handleExportPDF = () => {
    ...
    Object.entries(groupedStatistics).forEach(
      ([subject, students]: any, index) => {
        if (index !== 0) doc.addPage(); // new page per subject

        doc.setFontSize(14);
        doc.text(`Attendance Report - ${subject && ""}`, 14, 15);
        ...
        autoTable(doc, {
          ...
          styles: { fontSize: 10 },
          headStyles: { fillColor: [56, 178, 172] }, // teal header
        });
      }
    );
  ...
  const handleExportExcel = () => {
    ...
    Object.entries(groupedStatistics).forEach(([subject, students]: any) => {
      ...
      XLSX.utils.book_append_sheet(wb, ws, subject.substring(0, 30)); // sheet names max 31 chars
    });
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
              <div>
                ...
                <p className="text-gray-600">
                  Students with attendance below 85% threshold
                </p>
              </div>
