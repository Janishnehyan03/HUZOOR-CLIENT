...
const MarkedAttendaces: React.FC<MarkedAttendacesProps> = ({ attendances }) => {
  ...
  const sessionGroups = sortedAttendances.reduce((acc, curr) => {
    const session = curr.session || 1;
  ...
  return (
    <div className="space-y-8">
      {Object.entries(sessionGroups).map(([session, sessionAttendances]) => (
        <div key={session}>
          <h4 className="text-md font-semibold text-gray-700 mb-3 border-b pb-2">
            Session {session}
          </h4>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {(sessionAttendances as any[]).map((attendance) => {
                ...
                >
                  ...
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-gray-600 font-medium">
                      {attendance.student.class?.name || (attendance.student.admissionNumber ? `Adm: ${attendance.student.admissionNumber}` : "")}
                    </span>
                    {attendance.reason && (
